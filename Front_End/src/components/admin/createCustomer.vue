...
const submitForm = async () => {
    ...
    try {
        ...
        const response = await axios.post('http://localhost:5000/api/docgia', dataSend);

        notification.value = {
            message: 'Đọc giả đã được thêm thành công!',
            type: 'success'
        };
    } catch (error) {
        notification.value = {
        ...
        <div class="flex flex-col w-full">
            ...
            <div class="relative w-[95%] mx-auto h-[100%] my-5 overflow-hidden">
                <div class="text-center py-4">
                    <h2 class="text-[#333] font-bold text-[20px]">THÊM ĐỌC GIẢ</h2>
                </div>
